...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        dest="r_file",
        help="Input kraken report file for converting",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        dest="o_file",
        help="Output mpa-report file name",
    )
    parser.add_argument(
        "--display-header",
        action="store_true",
    ...
    main_lvls = ["superkingdom", "phylum", "class", "order", "family", "genus", "species"]
    domains = ["d__Eukarya", "d__Bacteria", "d__Eukarya", "d__Archaebacteria"]
    for line in r_file:
        ...
        if name[2] == "_":
            name = name[0:1] + name[2:]
        
        if level_type == "U":
            continue
        if level_type == "no rank": # root and unclassified
            continue
        # Create level name
        ...
        if level_type == "D" or (level_type == "x" and name in domains):
            ...
            level_str = name #level_type.lower() + "_" + name
